...
plot_upset <- function(genescore, save = FALSE, cutoff = 1, list.return = FALSE) {
  ...
  print(UpSetR::upset(pos.enrich.upset, order.by = 'freq', nsets = ncol(pos.enrich.upset),
  ...
  print(UpSetR::upset(neg.enrich.upset, order.by = 'freq', nsets = ncol(neg.enrich.upset),
                empty.intersections = 'on'))
  if(save){
